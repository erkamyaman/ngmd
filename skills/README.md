...
- **`ngmd-authoring`**: Writes and edits NgMd content. Covers the prose-vs-component page model, the seventeen authoring components (`<ngmd-callout>`, `<ngmd-alert>`, `<ngmd-card>`, `<ngmd-card-grid>`, `<ngmd-tabs>`, `<ngmd-workflow>`, `<ngmd-pill-row>`, `<ngmd-hero>`, `<ngmd-code-block>`, `<ngmd-accordion>`, `<ngmd-badge>`, `<ngmd-video>`, `<ngmd-image>`, plus the `<ngmd-pill>`, `<ngmd-step>`, `<ngmd-tab>`, `<ngmd-accordion-item>` children), the markdown affordances (`*Keyword` autolinks, `file="..."` code imports, group tabs, line highlighting), and the build-time link guards.
...
Both skills are scoped to working _inside_ an NgMd site. For working on the NgMd source itself, read the project's own `README.md` and `BACKLOG.md`.

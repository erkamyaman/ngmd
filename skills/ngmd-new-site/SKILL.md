...
| `src/app/services/toast/`         | `ToastService` plus `<app-toaster>` (mounted once in `app.ts`). Inject the service anywhere and call `.success('Saved.')`, `.error('Network failed.')`, or `.info('Heads up.')` to show a top-right toast. Service is a dumb queue; the component owns timing and slide-in / slide-out animation. SSR-safe.                                                                        |

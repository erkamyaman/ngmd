...
**Status-aware content banner.** New `<app-content-banners>` component renders a single banner above every documentation route when this deployment isn't the current stable. Reads `versions.self` (the entry for the live site) and `versions.current` (the entry whose `status` is `'current'`); when those differ, the banner copy and icon adapt to the self status — `next` ("You're reading the next-release docs"), `rc` ("You're reading a release candidate"), `deprecated` ("This is vX, no longer the current release"). The "see the current stable" link opens the current entry's URL in a new tab. No banner when self is current, when versions are not configured, or when there's no current entry registered.

**API reference auto-generation scaffold.** New `api-gen.plugin.ts` parses TypeScript sources via ts-morph based on an `ngmd.api.ts` scope file at the repo root, emits a `SymbolRecord` per exported declaration, and exposes the aggregate index as `virtual:ngmd/api-index`. New `/api` landing page renders every symbol grouped by `groupBy` strategy ('package' / 'directory' / 'kind'); `/api/<group>/<symbol>` per-symbol pages render signature, JSDoc body, and a source link to the GitHub line. Three rendering primitives under `src/app/ui/api/` (`<app-api-signature>`, `<app-api-jsdoc>`, `<app-api-badges>`). Cmd+K palette ingests the same index alongside content pages, so symbol search just works. Opt-in by dropping an `ngmd.api.ts` at the repo root; absent file means the plugin is a silent no-op.

**ROADMAP.md to 1.0.** New top-level document covers the path between 0.2.0 and a 1.0 release: two load-bearing features (API reference auto-generation, versioned docs), stability declaration (NgmdUi API freeze, CSS token freeze, plugin signature freeze), quality bar (test coverage, Lighthouse ≥95, axe-core sweep), polish bundle (frontmatter overrides, Mermaid, math, diff, print, reading time, keyboard nav), explicitly-punted items, sprint sequencing (~11-13 weeks total), and a definition-of-done checklist. i18n / locale routing was scoped, prototyped, and then explicitly dropped from the 1.0 cut because most Angular project docs are English-only; the work survives in a tracked plan but doesn't gate the release. `VERSION-SWITCHER-PLAN.md` captures the adev / PrimeNG model that this release implements.

...
const BOX =
  'px-5 py-4 rounded-lg border-y border-r border-t-zinc-200 border-r-zinc-200 border-b-zinc-200 dark:border-t-zinc-800 dark:border-r-zinc-800 dark:border-b-zinc-800 bg-zinc-50 dark:bg-zinc-900 border-l-[3px]';
...
@Component({
  ...
  template: `
    ...
    <ng-template #body>
      @if (title()) {
        <p class="font-semibold text-sm mt-0 mb-2 text-zinc-900 dark:text-zinc-100">
          {{ title() }}
        </p>
      }
      <div
        class="text-sm text-zinc-700 dark:text-zinc-300 [&>*:last-child]:mb-0 [&>*:first-child]:mt-0"
      >
